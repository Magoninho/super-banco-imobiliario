      ...
      .then((result) => {
        setPlayerListState(
          JSON.parse(result).filter(
            (elem) => elem.active != 0 && elem.player_id != currentPlayerId
          )
        );
      })
  ...
  return (
    ...
    >
      <fieldset>
        ...
        <div className="field border label prefix">
          ...
          >
            ...
            {playerListState.map((player) => (
              <option key={player.player_id} value={parseInt(player.player_id)}>
                {player.nickname}
              </option>
            ))}
          </select>
        ...
        <button className="responsive success">
          <i>sync_alt</i>Tranferir
        </button>
